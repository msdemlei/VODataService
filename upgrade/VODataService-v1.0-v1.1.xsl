...
<xsl:stylesheet xmlns:vr="http://www.ivoa.net/xml/VOResource/v1.0" 
   ...
   <xsl:template match="*" mode="VOResourceRoot">
      ...
      <xsl:element name="{$newResourceElement}" namespace="{$newResourceNS}">
         ...
         <xsl:for-each select="in-scope-prefixes(.)">
            <xsl:if test="string(.)!=$newResourceNS and 
                          namespace-uri-for-prefix(.,$curel)!=$vs10 and 
                          namespace-uri-for-prefix(.,$curel)!=$vs11">
              <xsl:namespace name="{.}" 
         ...
         <xsl:apply-templates select="*|text()" mode="vor">
            ...
            <xsl:with-param name="step" select="$step"/>
            <xsl:with-param name="pfx" select="$prefixes"/>
         </xsl:apply-templates>
   ...
   <xsl:template match="*[@xsi:type]" priority="1" mode="vor">
      ...
      <xsl:variable name="fp" select="concat($pfx,$prefix,'#')"/>

      <xsl:element name="{local-name()}">
         <xsl:for-each select="@*">
            <xsl:copy/>
         </xsl:for-each>
         <xsl:if test="not(contains($pfx,concat('#',$prefix,'#'))) and 
                       $prefix!='vs'">
