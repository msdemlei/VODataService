...
declare of the metadata of the table(s) underlying the service or
...
The \xmlel{vs:CatalogResource} resource type is used to descre a
...
distributed on the
sky, in energy, and in time.  For the energy coverage on the
electromagnetic spectrum, there is a
also qualitative classification in the \xmlel{waveband} element.
...
proposed in MOC 1.0 \citep{2014ivoa.spec.0602F}\footnote{Version 1.1 of MOC
will give a normative specification of ASCII MOCs, which is expected to 
exactly match the non-normative proposal in version 1.0.}
...
change the semantics?} to declare a URL giving a MOC 
...
resource may provide access several major astronomical catalogues
...
arraysize *, as supported by all VOTable readers.  VOTable readers
...
a VOTable FIELD's xtype attribute. Such information is reflected in
\xmlel{extendedType}.  
...
for describing an input parameter to a service or function.  In this
version of VODataService, input parameters are restricted to be defined
in terms of simple data types\todo{On the other hand,
...
The \xmlel{vs:InputParam} class then is a \xmlel{vs:DataType}
constrained to these types and furnished with additional metadata
