...
defined here into a relational structure.
\end{description}
...
children, the ``large-scale'' part is much harder to operationalise.
...
\lstinputlisting[language=XML,basicstyle=\footnotesize]{ipac-resource.xml}
...
instances.  These again have simple metadata, but mainly give column
metadata (including UCDs and units) in \xmlel{vs:TableParam}-typed
elements.  These are the basis for enabling discovery queries like
...
VODataService defines specialized interface type
...
\emph{name=value} pairs.  Such interface declarations should 
enumerate the input arguments that are supported by the service
implementation; this is particularly useful in the generation of simple
...
or standards), resource types are important.} resources should
nevertheless choose an appropriate type for their resources.  At the
...
example for this would be a couple of files on an FTP server.  CatalogX,
...
contrary, CatalogX should also be used for collection of images,
...
form-based access point would certainly not hurt).  Choose XService when
there is a 1:1 relationship between the publishing service and the
published data.


Two further resource classes in VODataService are deprecated in version
1.2.  
...
records using the deprecated types.  If all such records have
disappeared from the VO by version 1.3 of this specification, we may
remove the type declarations from the schema.
...
The type is built on top of \xmlel{vr:Service}, so they can have
...
primary access mode dedicated to the resource should use
...
access to (essentially) only the resource's data, as for instance an
``SSAP service for the XY instrument'', of for a service giving access
to multiple resources; in this latter case, make the resource-level
...
For DataService's content model, refer to sect.~\ref{sect:DataResource}.
...
resource.  In addition to \xmlel{vs:DataResource}, this type allows the
...
nonstandard.  Use \xmlel{vs:CatalogService} instead.
...
representations.  Its relationship is as between \xmlel{vs:DataResource}
...
For CatalogService's content model, refer to
sect.~\ref{sect:CatalogResource}.
...
used in new resource records.  Existing \xmlel{vs:DataCollection}-typed
records should be migrated to \xmlel{vs:DataResource} or
\xmlel{vs:CatalogResource} as appropriate.  The \xmlel{accessURL} child
of \xmlel{vs:DataCollection} (which is not part of any of the new
classes) should be replaced by a plain capability with a
...
no properly standardised way of creating such records.
...
qualitative classification in the \xmlel{waveband} \todo{Worry 
...
tables together that are logically related.\todo{Better cater for the
case that tables are outside of all schemas (see SIMBAD)}  For example, a single
